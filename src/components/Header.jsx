       ...
       <nav className="flex items-center gap-4 text-[12.8px] font-bold text-secondary">
        {navItems.map((item, index) => (
          ...
          >
            ...
            {/* Label */}
            {item.label && (
              <span className="hover:bg-[#25B003] hover:text-white py-1 px-2 transition-all duration-200">
                {item.label}
              </span>
            )}
         ...
         <div className="relative flex items-center gap-1 text-[12.8px] font-bold cursor-pointer" onClick={() => setActivePanel(activePanel === "business" ? null : "business")}>
          ...
          {actionBtns.map((btn, index) => (
            <span
              ...
              onClick={() => {
                if (btn.icon === "device_reset") {
                  setActivePanel(activePanel === "reset" ? null : "reset");
                }
              }}
            >
              {btn.icon}
    ...
    <div className="p-5">
      ...
      {activePanel === "business" && (
        <>
          ...
          <button className="w-full border border-[#25B003] text-[#25B003] rounded-md py-2 text-[12.8px] font-bold hover:bg-[#25B003] hover:text-white transition">
