...
export default function EditVoucherSeries() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f6f8] px-10 py-6">
      ...
      <div className="bg-white border rounded-md p-8 w-full max-w-[1000px]">
        ...
        {/* SERIES NAME */}
        <FormRow label="Series Name" required defaultValue="Main Series" />

        {/* DEFAULT BANK */}
        <FormSelect label="Default Bank" placeholder="Choose" defaultValue="" />

        {/* VOUCHER TYPE */}
        <FormSelect label="Voucher Type" required defaultValue="Contra" />
        ...
        <div className="grid grid-cols-[220px_1fr] items-center mb-6">
          ...
          <div className="flex items-center gap-8 text-[14px] font-semibold">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="numbering" defaultChecked />
              Automatic
            </label>
        ...
        <div className="flex justify-center gap-6 mt-8">
          ...
          <button
            onClick={() => navigate(-1)}
            className="px-8 h-[38px] bg-[#2d3748] text-white font-bold rounded-md text-tiny"
          >
            QUIT
          </button>
        </div>
...
const FormSelect = ({ label, required, placeholder, defaultValue }) => (
  <div className="grid grid-cols-[220px_1fr] items-center mb-6">
    <label className="text-[14px] font-semibold">
      {label}
      {required && <span className="text-red-500 ml-1">*</span>}
