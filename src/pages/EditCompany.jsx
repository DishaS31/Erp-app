...
const parseDate = (val) => {
  // supports "2025-04-01" or "01-04-2025"
  if (!val) return null;
...
export default function EditCompany() {
  const { id } = useParams(); // /company/edit/:id
  ...
  const [fyStart, setFyStart] = useState(new Date(2025, 3, 1)); // default
 ...
 const [form, setForm] = useState({
  ...
  useEffect(() => {
    ...
    console.log("companyinfo raw response:", data);

        const c = data?.data || {};

       // fill form with API values (mapping)
        setForm((prev) => ({
        ...prev,

        company_name: c?.comp_name?.trim() || "",
        print_name: c?.print_name?.trim() || "",
        short_name: c?.short_name?.trim() || "",

        ro_address1: c?.ro_adrs1 || "",
        ro_address2: c?.ro_adrs2 || "",

        // ✅ API gives IDs like "1"
        ro_country: c?.ro_country || "1",
        ro_state: c?.ro_state || "18",

        ro_city: c?.ro_city || "",
        ro_pin: c?.ro_pin || "",

        // ✅ API gives "AVG COST" but your select uses "AVG"
        stock_valuation:
            c?.default_stock === "AVG COST" ? "AVG COST" : (c?.default_stock || "AVG"),

        industry_type: c?.industry_type || "",
        nature_of_work: c?.nature_of_work || "",

        co_address1: c?.co_adrs1 || "",
        co_address2: c?.co_adrs2 || "",

        // ✅ API gives IDs like "1"
        co_country: c?.co_country || "1",
        co_state: c?.co_state || "18",

        co_city: c?.co_city || "",
        co_pin: c?.co_pin || "",
        }));

        const fy = parseDate(c?.fy_start);
        if (fy) setFyStart(fy);
 ...
 const handleUpdate = async () => {
  try {
    ...
    const payload = {
      ...
      comp_name: form.company_name?.trim(),
      ...
      // ✅ FIXED
      default_stock: form.stock_valuation === "AVG" ? "AVG COST" : form.stock_valuation,
      ...
      ro_country: form.ro_country,  // must be ID
      ro_state: form.ro_state,      // must be ID
      ro_city: form.ro_city,
      ro_pin: form.ro_pin,

      co_adrs1: form.co_address1,
      co_adrs2: form.co_address2,
      co_country: form.co_country,  // must be ID
      co_state: form.co_state,      // must be ID
  ...
  return (
    <div className="p-6 pt-0 min-h-screen text-[#141824] font-sans">
      <h1 className="text-2xl font-extrabold mb-6">
        Update Company (ID: {id})
      </h1>

      {loading && (
        <div className="p-3 mb-4 rounded bg-yellow-100 text-yellow-800 font-semibold">
          Loading company info...
        </div>
      )}

      {error && (
        <div className="p-3 mb-4 rounded bg-red-100 text-red-700 font-semibold">
          {error}
        </div>
      )}
      ...
      >
          ...
          <section className="bg-white p-5 rounded-xl shadow-sm border border-[#cbd0dd] focus-input">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 flex gap-1 items-center">
                <label className="text-sm font-semibold w-28">
                  Address line 1
                </label>
                <input
                  name="ro_address1"
              ...
              <div className=" flex gap-1 items-center">
                ...
                <select
                  name="ro_country"
                  required
                  value={form.ro_country}
                  onChange={handleChange}
                  className="w-full border rounded-md px-3 py-2 border-[#cbd0dd] focus-input bg-white font-semibold"
                >
        ...
        <div className="flex justify-center gap-4 mt-6">
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-2.5 bg-primary text-white rounded-md font-bold text-tiny disabled:opacity-60"
          >
          ...
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="px-6 py-2.5 bg-gray-900 text-white rounded-md font-bold text-tiny"
          >
            QUIT
          </button>
        </div>
